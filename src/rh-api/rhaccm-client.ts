...
import type { paths } from './gen/rhaccm.d.ts';
...
export type AccessTokenResponse =
  paths['/api/accounts_mgmt/v1/access_token']['post']['responses']['200']['content']['application/json'];
...
export class AccountManagementV1 {
  ...
  async getAccessToken(): Promise<AccessToken> {
    ...
    if (data.auths?.['registry.redhat.io']) {
      authToken = atob((data.auths['registry.redhat.io'] as AuthConfig).auth);
    }
    if (!authToken) {
      throw new Error('Failed to obtain auth token for registry.redhat.io');
    }
    const [username, password] = authToken.split(':');
    if (!username || !password) {
      throw new Error('Failed to obtain usename and password for registry.redhat.io');
    }
